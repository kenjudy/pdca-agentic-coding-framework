...
- [ ] No untested implementation was committed — show git diff of implementation vs test files
- [ ] Simple test scenarios were effective
- [ ] For any acceptance criteria with an end-to-end/completeness claim, or any change spanning more than one file, was an adversarial critic pass run with a fresh subagent, model chosen by the operator? When the operator has explicitly instructed you to act autonomously for this session, skip asking and default instead: fresh subagent, no third-party review skill, on a different model at the same or higher reasoning tier than the model that did the work. Absent that instruction, present the choice to the operator and wait. (see `references/testing-anti-patterns.md` #8; a self-review or re-read by the same session that did the work is not a substitute — it is exactly what has repeatedly missed these findings, and a different subagent on the *same* model at the ceiling is not an exception to that: freshness of context is what made the shipped critic pass effective, not strictly higher rank)
