...
- [ ] For any acceptance criteria with an end-to-end/completeness claim, or any change spanning more than one file, was an adversarial critic pass run with a fresh subagent, model chosen by the operator? (see `references/testing-anti-patterns.md` #8; a self-review or re-read by the same session that did the work is not a substitute — it is exactly what has repeatedly missed these findings)
