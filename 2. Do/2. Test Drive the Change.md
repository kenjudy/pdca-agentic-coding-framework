...
**If ponytail is active** (`/ponytail` is a recognized command in this session): ponytail governs what gets built and how complex it is; PDCA governs how it is verified. The rules below already resolve every conflict between them — see `references/ponytail-workflow.md` for the precedence rules stated explicitly.

**🚨 CALLED SHOT — MANDATORY BEFORE EVERY TEST 🚨**

YOU MUST output all six before writing or running any test:
...
- **Oracle:** [where does the expected value come from — a literal, observable state (a file on disk, an HTTP response, a database row) or an external tool, independent of the code under test? Re-deriving it with the same logic under test does not qualify]
